...
#mmr骨骼属性类（legacy版本，使用独立属性名避免冲突）
class MMR_Legacy_Bone(bpy.types.PropertyGroup):
...
# 这些属性在 register() 中注册，不在模块级别注册
# 定义骨骼头尾是否反转
# bpy.types.PoseBone.mmr_bone_invert = ...
# 定义骨骼类型
# bpy.types.PoseBone.mmr_bone_type = ...
mmr_bone_property_list=[name for name in MMR_Legacy_Bone.__annotations__.keys()]
...
def get_preset(pose):
    ...
    for bone in pose.bones:
        if bone.mmr_bone_type and bone.mmr_bone_type != 'None':
...
class OT_QA_Start(bpy.types.Operator):
    ...
    def execute(self,context):
        ...
        bones = context.view_layer.objects.active.data.bones
        if len(bones) == 0:
            return{"CANCELLED"}
        first_bone = bones[0]
        # Blender 4.0+ 中 Bone.select 已移除，通过设置 active bone 来选择
...
class Mmr_Preset_Panel_Base:
    """Legacy 预设面板基类（不继承 Panel，避免被 auto_load 误注册）"""
...
def _legacy_preset_poll(context):
    """Legacy 预设面板通用 poll"""
...
@reg_order(0)
class MMR_Arm_Panel(Mmr_Preset_Panel_Base, bpy.types.Panel):
    ...
    def poll(cls, context):
        return _legacy_preset_poll(context)
    ...
    def draw(self, context):
        ...
        if mmr_property.quick_assign_mod:
            ...
            if mmr_property.extra_options1:
                layout.prop(mmr_property,'preferences',text=i18n("Preference"), toggle=True)
                if mmr_property.preferences:
                    box = layout.box()
                    ...
                    row.prop(prefs, "left_leg_ik_fk_preference",
                             text=i18n("Left Leg IK" if prefs.left_leg_ik_fk_preference else "Left Leg FK"),
                             toggle=True)
                    row.prop(prefs, "right_leg_ik_fk_preference",
                             text=i18n("Right Leg IK" if prefs.right_leg_ik_fk_preference else "Right Leg FK"),
                             toggle=True)
                ...
                layout.prop(prefs, "bent_IK_bone", text=i18n("Bent IK bone"))
                layout.prop(prefs, "auto_shoulder", text=i18n("Shoulder IK"))
                layout.prop(prefs, "pole_target", text=i18n("Use pole target"))

                layout.prop(mmr_property,'use_itasc_solver',text="Use ITASC Solver")
                layout.prop(mmr_property,'hide_mmd_skeleton',text="Hide MMD Skeleton")


class MMR_Bone_Panel(Mmr_Preset_Panel_Base, bpy.types.Panel):
    ...
    @classmethod
    def poll(cls, context):
        if not _legacy_preset_poll(context):
            return False
...
class MMR_Retarget_Panel(Mmr_Preset_Panel_Base, bpy.types.Panel):
...
def register():
    """注册 legacy 版本的 PoseBone 属性（类由 auto_load 自动注册）"""
    if not hasattr(bpy.types.PoseBone, 'mmr_legacy_bone'):
        bpy.types.PoseBone.mmr_legacy_bone = bpy.props.PointerProperty(type=MMR_Legacy_Bone)
    if not hasattr(bpy.types.PoseBone, 'mmr_bone_type'):
        # 构建枚举项列表，使用 bone_translate_dict_C 提供完整覆盖和中文名称
        _bone_type_items = [(name, bone_translate_dict_C.get(name, name), "") for name in bone_translate_dict_C]
        bpy.types.PoseBone.mmr_bone_type = EnumProperty(
            items=_bone_type_items,
            default='None',
            description='Choose the bone type you want to use'
        )
    if not hasattr(bpy.types.PoseBone, 'mmr_bone_invert'):
        bpy.types.PoseBone.mmr_bone_invert = BoolProperty(
            default=False,
            description='Invert bone head/tail'
        )

def unregister():
    """注销 legacy 版本的 PoseBone 属性（类由 auto_load 自动注销）"""
    if hasattr(bpy.types.PoseBone, 'mmr_bone_type'):
        del bpy.types.PoseBone.mmr_bone_type
    if hasattr(bpy.types.PoseBone, 'mmr_bone_invert'):
        del bpy.types.PoseBone.mmr_bone_invert
    if hasattr(bpy.types.PoseBone, 'mmr_legacy_bone'):
        del bpy.types.PoseBone.mmr_legacy_bone
