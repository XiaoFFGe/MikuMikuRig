...
def retarget_mixmao(OT,context):
    ...
    # 兼容新版本RIG.py生成的控制器：新版本不会在rigify骨骼上设置mmr_bone_type，
    # 因此需要先应用Rigify预设来补充骨骼类型映射
    rigify_preset = preset.preset_dict_dict['retarget']['Rigify']
    preset.set_bone_type(rigify_arm.pose, rigify_preset)
    # 新版本将Center骨骼重命名为torso_root，补充映射
    if 'torso_root' in rigify_arm.pose.bones:
        rigify_arm.pose.bones['torso_root'].mmr_bone_type = 'Center'
...
def load_vmd(OT,context):
    ...
    # 因此需要先应用Rigify预设来补充骨骼类型映射
    rigify_preset = preset.preset_dict_dict['retarget']['Rigify']
    preset.set_bone_type(rigify_arm2.pose, rigify_preset)
    # 新版本将Center骨骼重命名为torso_root，补充映射
    if 'torso_root' in rigify_arm2.pose.bones:
        rigify_arm2.pose.bones['torso_root'].mmr_bone_type = 'Center'
    rigify_dict={}
    ...
    if mmr_property.auto_action_scale:
        if 'thigh.L' not in rigify_dict:
            print('Warning: thigh.L not found in rigify_dict, skip auto scale')
        else:
    ...
    #矫正手臂角度
    if 'upper_arm.L' not in rigify_dict or 'forearm.L' not in rigify_dict:
        print('Warning: upper_arm.L or forearm.L not found in rigify_dict, skip arm angle correction')
    else:
    ...
    def copy_fcurve(FK_type):
        if FK_type not in rigify_dict:
            print(f'Warning: {FK_type} not found in rigify_dict, skip copy_fcurve')
            return
    ...
    #clear certain keyframe
    for bone in rigify_arm2.pose.bones:
        # Blender 4.0+ 中 Bone.hide 已移除，用 getattr 安全获取
        if getattr(bone.bone, 'hide', False) == False:
    ...
    def match_bone(from_type,to_name_list):
        if from_type not in rigify_dict:
            print(f'Warning: {from_type} not found in rigify_dict, skip match_bone')
            return
    ...
    for name in bake_name_list:
        # Blender 4.0+ 中 Bone.select 已移除，改用 pose.bones
        try:
            mmd_arm.pose.bones[name].select = True
    ...
    bpy.ops.nla.bake(frame_start=int(frame_range[0]),
...
def export_vmd(OT,context):
    ...
    mmd_rigify_dict={
    '全ての親':['root'],'センター':['spine','torso_root','Center'],'下半身':['spine_fk','hips'],'上半身':['spine_fk.001','spine_fk.002','chest'],'上半身2':['spine_fk.003','chest'],
    ...
    #duplicate armature
    mmd_arm=None
    # 兼容新旧版本命名：旧版用 xxx_rig，新版用 RIG-xxx
    mmd_name = rigify_arm.name.replace('_rig', '')
    if mmd_name not in bpy.data.objects.keys():
        mmd_name = rigify_arm.name.replace('RIG-', '', 1)
    ...
    rigify_action = rigify_arm.animation_data.action if rigify_arm.animation_data else None

    if set_action_range:
        start_frame1=start_frame
        end_frame1=end_frame
    else:
        if rigify_action is not None:
            start_frame1=rigify_action.frame_range[0]
            end_frame1=rigify_action.frame_range[1]
        else:
            # 没有动作时使用场景帧范围
            start_frame1 = context.scene.frame_start
            end_frame1 = context.scene.frame_end
    ...
    for bone in mmd_arm2.pose.bones:
        ...
        if name_j in mmd_rigify_dict:
            # Blender 4.0+ 中 Bone.select 已移除，改用 PoseBone.select
            bone.select = True
            mmd_blender_dict[name_j]=bone.name
        else:
            bone.select = False

    bpy.ops.nla.bake(frame_start=int(start_frame1),
                     frame_end=int(end_frame1),
                     only_selected=True,
                     visual_keying=True,
                     bake_types={'POSE'},
                     )
    ...
    mmd_action = mmd_arm2.animation_data.action if mmd_arm2.animation_data else None
    fcurves1 = rigify_action.fcurves if rigify_action else None
    fcurves2 = mmd_action.fcurves if mmd_action else None
    ...
    def clean_fcurve(blender_name,keyframe_bone_list):
        ...
        if fcurves1 is None or fcurves2 is None:
            return
        ...
        def gather_data(bone_name,attr_name):
            bone = rigify_arm.pose.bones.get(bone_name)
            if bone is None:
                return
