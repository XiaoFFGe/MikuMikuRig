...
# ---- Blender 4.0+ 骨骼集合兼容层 ----
# 旧版 API bone.layers / armature.layers 在 Blender 4.0+ 中已移除，
# 替换为 BoneCollection 系统。

def _get_or_create_collection(armature, layer_index):
    """根据层索引获取或创建骨骼集合"""
    coll_name = f"Layer_{layer_index}"
...
def _copy_bone_collections(source_bone, target_bone):
    """将 source_bone 的集合复制到 target_bone"""
    for coll in source_bone.collections:
        coll.assign(target_bone)


def _set_bone_collection(bone, layer_index):
    """将骨骼分配到指定层索引的集合，并从其他集合中移除"""
    armature = bone.id_data
    coll = _get_or_create_collection(armature, layer_index)
    # 从所有集合中移除
...
def _unhide_bone(bone):
    """显示骨骼（从隐藏集合中移除）"""
    armature = bone.id_data
    coll = armature.collections.get("_MMR_Hidden")
    if coll is not None:
        coll.unassign(bone)


def compare_version(version1, version2):
    """比较两个版本号，version1 < version2 返回 True"""
    parts1 = []
    parts2 = []
...
def add_constraint3(constraint_List,preset_dict):
    ...
    for i,tuple in enumerate(constraint_List):
        ...
        if From in mmd_bones_list and To in rig_bones_list:
            match_bone_nunber+=1
            index_list.append(i)
            _unhide_bone(mmd_arm.data.bones[From])
    ...
    for i in index_list:
        ...
        _copy_bone_collections(rig.data.edit_bones[To], parent_bone)
...
def RIG2(context):
    ...
    bent_finger=False
    if all(k in preset_dict for k in ['f_index.01.L', 'f_index.02.L', 'f_index.03.L']):
    ...
    #检测手臂弯曲
    bent_arm=False
    if all(k in preset_dict for k in ['upper_arm.L', 'forearm.L', 'hand.L']):
    ...
    if prefs.auto_shoulder:
        ...
        _copy_bone_collections(rig.data.bones["ORG-shoulder.L"], auto_shulder_L.bone)
        ...
        _copy_bone_collections(rig.data.bones["ORG-shoulder.R"], auto_shulder_R.bone)
    ...
    if mmr_property.upper_body_controller:
        ...
        _copy_bone_collections(rig.data.bones["torso"], Center.bone)
        Center.bone.color.palette = 'THEME09' 
    else:
    ...
    if prefs.neck_follow:
        ...
        rig.pose.bones["torso"]["neck_follow"] = 0

    # 头部跟随
    if prefs.head_follow:
        rig.pose.bones["torso"]["head_follow"] = 1
    else:
        rig.pose.bones["torso"]["head_follow"] = 0

    # 双眼跟随
    if prefs.both_eye_follow:
        rig.pose.bones["eyes"]["eyes_follow"] = 1
    else:
        rig.pose.bones["eyes"]["eyes_follow"] = 0

    # 根据 Blender 版本使用不同方式隐藏特定骨骼
    ...
    if compare_version(blender_version, "4.9.9"):
        for n in not_bone:
            bone = rig.data.bones.get(n)
            if bone:
                bone.hide = True
    else:
        for n in not_bone:
            bone = rig.pose.bones.get(n)
            if bone:
                bone.hide = True
    ...
    # 关闭IK拉伸
    for i in ik_stretch:
        rig.pose.bones[i]["IK_Stretch"] = 0

    # FK跟随
    for i in ik_stretch:
        if prefs.arm_to_leg_following:
            rig.pose.bones[i]["FK_limb_follow"] = 0
        else:
            rig.pose.bones[i]["FK_limb_follow"] = 1

    # IK-FK偏好设置
    for i in ik_stretch:
        if i == "upper_arm_parent.L":
            if prefs.left_ik_fk_preference:
                rig.pose.bones[i]["IK_FK"] = 0
            else:
                rig.pose.bones[i]["IK_FK"] = 1
    ...
    # 极向目标
    if prefs.pole_target:
        for i in ik_stretch:
            bone = rig.pose.bones.get(i)
            if bone:
    ...
    arms = {}
    bone_names = ['upper_arm_fk.L', 'forearm_fk.L', 'hand_fk.L', '>---<',
    ...
    for bone1_name, bone2_name in arms.items():
        bone1 = rig.data.edit_bones.get(bone1_name)
        bone2 = rig.data.edit_bones.get(bone2_name)
    ...
    # 隐藏部分控制器集合
    is_gto = ['Face (Primary)', 'Face (Secondary)', 'Torso (Tweak)', 'Torso (Redirect)',
              'Fingers (Detail)', 'Fingers (IK)',
              'Arm.L (FK)', 'Arm.R (FK)',
              'Arm.L (Tweak)', 'Arm.R (Tweak)',
              'Leg.L (FK)', 'Leg.R (FK)',
              'Leg.L (Tweak)', 'Leg.R (Tweak)']
    for n in is_gto:
        if n in rig.data.collections_all:
            rig.data.collections_all[n].is_visible = False

    # IK-FK偏好骨骼集合显示
    if prefs.left_ik_fk_preference:
        if 'Arm.L (IK)' in rig.data.collections_all:
            rig.data.collections_all['Arm.L (IK)'].is_visible = True
        if 'Arm.L (FK)' in rig.data.collections_all:
            rig.data.collections_all['Arm.L (FK)'].is_visible = False
    else:
        if 'Arm.L (IK)' in rig.data.collections_all:
            rig.data.collections_all['Arm.L (IK)'].is_visible = False
        if 'Arm.L (FK)' in rig.data.collections_all:
            rig.data.collections_all['Arm.L (FK)'].is_visible = True

    if prefs.right_ik_fk_preference:
        if 'Arm.R (IK)' in rig.data.collections_all:
            rig.data.collections_all['Arm.R (IK)'].is_visible = True
        if 'Arm.R (FK)' in rig.data.collections_all:
            rig.data.collections_all['Arm.R (FK)'].is_visible = False
    else:
        if 'Arm.R (IK)' in rig.data.collections_all:
            rig.data.collections_all['Arm.R (IK)'].is_visible = False
        if 'Arm.R (FK)' in rig.data.collections_all:
            rig.data.collections_all['Arm.R (FK)'].is_visible = True

    if prefs.left_leg_ik_fk_preference:
        if 'Leg.L (IK)' in rig.data.collections_all:
            rig.data.collections_all['Leg.L (IK)'].is_visible = True
        if 'Leg.L (FK)' in rig.data.collections_all:
            rig.data.collections_all['Leg.L (FK)'].is_visible = False
    else:
        if 'Leg.L (IK)' in rig.data.collections_all:
            rig.data.collections_all['Leg.L (IK)'].is_visible = False
        if 'Leg.L (FK)' in rig.data.collections_all:
            rig.data.collections_all['Leg.L (FK)'].is_visible = True

    if prefs.right_leg_ik_fk_preference:
        if 'Leg.R (IK)' in rig.data.collections_all:
            rig.data.collections_all['Leg.R (IK)'].is_visible = True
        if 'Leg.R (FK)' in rig.data.collections_all:
            rig.data.collections_all['Leg.R (FK)'].is_visible = False
    else:
        if 'Leg.R (IK)' in rig.data.collections_all:
            rig.data.collections_all['Leg.R (IK)'].is_visible = False
        if 'Leg.R (FK)' in rig.data.collections_all:
            rig.data.collections_all['Leg.R (FK)'].is_visible = True

    # 隐藏无眼睛时的面部控制器
    if 'eye.L' not in preset_dict or 'eye.R' not in preset_dict:
        if 'Face (Primary)' in rig.data.collections_all:
            rig.data.collections_all['Face (Primary)'].is_visible = False

    # 使用ITASC解算器
    if mmr_property.use_itasc_solver:
        rig.pose.ik_solver = 'ITASC'

    # 锁定移动的骨骼
    lock_location_bone_list = [
        "upper_arm_ik.L", "upper_arm_ik.R", "thigh_ik.L", "thigh_ik.R",
    ...
    lock_scale_bone_list = [
        ...
        "hand_ik.L", "hand_ik.R", "upper_arm_ik.L", "upper_arm_ik.R",
        "thigh_ik.L", "thigh_ik.R", "hips", "chest", "neck", "head",
        "shoulder.L", "shoulder.R"
    ]
    ...
    # 隐藏原骨架
    if mmr_property.hide_mmd_skeleton:
        mmd_arm.hide_set(True)
    else:
        mmd_arm.select_set(True)

    # 删除临时metarig对象
    del_object = bpy.data.objects.get(rigify_arm_name)
    if del_object:
        bpy.ops.object.select_all(action='DESELECT')
    ...
    rig.matrix_world = mmd_arm.matrix_world  # 吸附位置
...
def decorate_mmd_arm(context):
    ...
    def set_bone_layer(bone,number):
        _set_bone_collection(bone, number)
    ...
    # 设置骨骼颜色（Blender 4.0+ 使用 bone.color.palette 替代 bone_groups）
    # 先创建骨骼集合并分配颜色
    for name_j, name in mmd_dict.items():
        if name_j in bone_color_dict:
            # 确保集合存在
            coll = mmd_arm.data.collections.get(name_j)
            if coll is None:
                coll = mmd_arm.data.collections.new(name_j)
            coll.assign(mmd_arm.data.bones[name])
            # 设置骨骼颜色
            mmd_arm.data.bones[name].color.palette = bone_color_dict[name_j]

    # 设置骨骼层（Blender 4.0+ 使用集合）
    for pose_bone in pose_bones:
        bone = pose_bone.bone
