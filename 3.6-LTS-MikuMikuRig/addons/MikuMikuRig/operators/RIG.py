    ...
    def execute(self, context: bpy.types.Context):
        ...
        for key, value in config.items():
            ...
            if key in mmd_edit_bones and value in rigify_edit_bones:
                ...
                # 隐藏骨骼
                bone = rigify.data.bones.get(value + '_parent')
                bone.hide = True
            else:
        ...
        is_gto = [1, 2, 4, 6, 8, 11, 9, 12, 14, 17, 15, 18]

        # 隐藏骨骼集合
        for n in is_gto:
            rigify.data.layers[n] = False
...
class mmrexportvmdactionsOperator(bpy.types.Operator):
    ...
    def execute(self, context: bpy.types.Context):
        ...
        # 检查所有的骨骼，如果名称里面有"MMR"的骨骼约束，则选中
        armature = context.object
        for bone in armature.pose.bones:
            for constraint in bone.constraints:
                if "MMR_" in constraint.name:
...
# 添加权重骨骼项
class MMR_OT_Weight_Bone_Parent_Add(bpy.types.Operator):
    bl_idname = "mmr.weight_bone_parent_add"
    bl_label = ""
    bl_description = "添加项"
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def execute(self, context):
        ...
        items = obj.mmr_weight_bone_parent_fix
        ...
        obj.mmr_weight_bone_parent_fix_index = len(items) - 1 if len(items) > 0 else -1

        return {'FINISHED'}

# 删除权重骨骼项
class MMR_OT_Weight_Bone_Parent_Del(bpy.types.Operator):
    bl_idname = "mmr.weight_bone_parent_del"
    bl_label = ""
    bl_description = "删除项"
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def execute(self, context):
        ...
        # 删除当前项
        items.remove(obj.mmr_weight_bone_parent_fix_index)

        # 更新索引
        if obj.mmr_weight_bone_parent_fix_index >= len(items):
            obj.mmr_weight_bone_parent_fix_index = len(items) - 1 if len(items) > 0 else -1

        return {'FINISHED'}

# 导入默认权重骨骼项
class MMR_OT_Import_Default_Weight_Bone_Parent(bpy.types.Operator):
    bl_idname = "mmr.import_default_weight_bone_parent"
    bl_label = ""
    bl_description = "导入默认权重骨骼项"
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def execute(self, context):
        ...
        # 导入默认项
        for k, v in bone_to_d_bone.items():
            item = items.add()
            item.key = k
            item.value = v
