...
# XF刚体转换MMR刚体
class xf_rigidbody_to_mmrrigidbody(bpy.types.Operator):
    '''xf_rigidbody_to_mmrrigidbody'''
    bl_idname = "mmr.xf_rigidbody_to_mmr_rigidbody"
    bl_label = "XF Rigidbody to MMR Rigidbody"
    bl_description = "XF刚体转MMR刚体"
    bl_options = {'REGISTER', 'UNDO'}  # 启用撤销功能
        ...
        if children_bones:
            ...
            for obj in children_bones:
                if obj.type == 'MESH':  # 只处理网格对象
                    if obj.rigid_body:
                        for j in range(16):
                            if j < len(obj.rigid_body.xf_col_group_mask):
                                obj.mmr_bone.collision_group_mask[j] = obj.rigid_body.xf_col_group_mask[j]
                            else:
                                obj.mmr_bone.collision_group_mask[j] = False
                    
                        obj.mmr_bone.collision_group_index = obj.rigid_body.xf_col_group_idx
    ...
    def execute(self, context):
        ...
        for obj in collection.children:
            if obj.type == 'MESH': # 只处理网格对象
                ...
                for i in range(20):
                    if i < len(obj.mmr_bone.collision_group_mask):
                        obj.rigid_body.xf_col_group_mask[i] = obj.mmr_bone.collision_group_mask[i]
                    else:
                        obj.rigid_body.xf_col_group_mask[i] = False

                obj.rigid_body.xf_col_group_idx = obj.mmr_bone.collision_group_index
        ...
        print('-' * 20)
        self.report({'INFO'}, f"共创建{idxs}个刚体穿插对")
        return {'FINISHED'}
